...
async def _process_email_batch(session, emails: List, coordinator: EmailProcessingCoordinator, user_id: str, job: EmailTransactionSyncJob):
...
def process_sms_batch_task(self, user_id: str, messages_data: List[dict]):
    ...
    try:
        return loop.run_until_complete(_process_sms_batch_async(user_id, messages_data))
    except Exception as exc:
        logger.error(f"SMS batch task failed for user {user_id}: {exc}", exc_info=True)
        raise self.retry(exc=exc, countdown=60 * (self.request.retries + 1))


async def _process_sms_batch_async(user_id: str, messages_data: List[dict]):
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            # Process each SMS
            await _process_sms_messages(session, messages_data, coordinator, user_id, job)
...
async def _process_sms_messages(
    session, 
